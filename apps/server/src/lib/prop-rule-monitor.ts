...
function formatMetricValue(value: number, mode: "currency" | "percent") {
  return mode === "currency" ? `$${value.toFixed(2)}` : `${value.toFixed(2)}%`;
}

function formatPropRuleLabel(rule: string) {
  switch (rule) {
    case "daily_loss":
      return "Daily DD";
    case "max_loss":
...
function buildPropAlertNotification(
  account: {
    id: string;
    userId: string;
    name: string;
  ...
  alert: RuleCheckResult["alerts"][number]
) {
...
function getClosedTradeNetPnl(row: typeof trade.$inferSelect) {
  return toNumber(row.profit) + toNumber(row.commissions) + toNumber(row.swap);
}

function getOpenTradeNetPnl(row: typeof openTrade.$inferSelect) {
...
function getRealizationDayKey(row: typeof trade.$inferSelect | null) {
  if (!row) return null;
  const sourceDate = row.closeTime || row.openTime;
  return sourceDate ? toDayKey(sourceDate) : null;
}

function getPhaseTargetAbsolute(
  phase: Pick<PropPhaseRules, "profitTarget" | "profitTargetType">,
