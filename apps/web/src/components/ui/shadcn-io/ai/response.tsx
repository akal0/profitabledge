...
const components: Options['components'] = {
  ...
  hr: ({ node, className, ...props }) => (
    <hr className={cn('my-6 border-white/5', className)} {...(props as any)} />
  ),
  ...
  a: ({ node, children, className, ...props }) => {
    ...
    if (href?.startsWith('mention:')) {
      return (
        <span
          className={cn(
            'mention inline-flex items-center rounded px-1.5 py-0.5 text-xs font-medium bg-emerald-500/20 text-emerald-200',
            className
          )}
          data-type="mention"
        >
    ...
    if (href?.startsWith('command:')) {
      return (
        <span
          className={cn(
            'mention inline-flex items-center rounded px-1.5 py-0.5 text-xs font-medium bg-amber-500/20 text-amber-200',
            className
          )}
          data-type="command"
        >
    ...
    return (
      <a
        className={cn('font-medium text-purple-400 hover:text-purple-300 underline', className)}
        rel="noreferrer"
        target="_blank"
        {...(props as any)}
      >
  ...
  table: ({ node, children, className, ...props }) => (
    <div className="my-4 overflow-x-auto">
      <table
        className={cn('w-full border-collapse border border-white/10', className)}
        {...(props as any)}
      >
        {children}
      </table>
  ...
  thead: ({ node, children, className, ...props }) => (
    <thead className={cn('bg-sidebar/50', className)} {...(props as any)}>
      {children}
    </thead>
  ),
  tbody: ({ node, children, className, ...props }) => (
    <tbody className={cn('divide-y divide-white/5', className)} {...(props as any)}>
      {children}
    </tbody>
  ),
  tr: ({ node, children, className, ...props }) => (
    <tr className={cn('border-white/5 border-b', className)} {...(props as any)}>
      {children}
    </tr>
  ),
  ...
  blockquote: ({ node, children, className, ...props }) => (
    <blockquote
      className={cn(
        'my-4 border-white/20 border-l-4 pl-4 text-white/70 italic',
        className
      )}
      {...(props as any)}
    >
  ...
  code: ({ node, className, ...props }) => {
    ...
    return (
      <code
        className={cn(
          'rounded bg-sidebar/50 px-1.5 py-0.5 font-mono text-xs text-purple-300 border border-white/10',
          className
        )}
        {...(props as any)}
      />
