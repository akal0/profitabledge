...
- live account-equity floating P&L now follows net open P&L including swap from `accounts.liveMetrics`, so the floating figure and any fallback equity math stay aligned with open MT4/MT5 positions instead of using raw position profit alone
...
- the daily briefing widget now formats its header date as `Tue 24th March '26` and its review stats follow the latest closed-trade day when strict “yesterday” would be empty, so the briefing card no longer collapses to misleading zeroes just because the prior calendar day had no closed trades
- dashboard widgets now expose both card-level and section-level share actions; single-widget exports still capture the current rendered card with user-driven resize state, while the header-level widgets export captures the full selected overview widget grid, and every PNG export now lands on a generously padded `bg-sidebar` canvas with inline chart/svg colors and crisp ring-style control chrome preserved so chart grids, guide dashes, and widget chrome match the live dashboard instead of brightening during capture or hugging the PNG edges
- the main trading calendar and economic calendar now reuse the shared PNG export flow, so the active month, week, day, or list calendar layout can be downloaded straight from the calendar header without route-local screenshot code; the trading calendar export keeps the picker and view/overlay segmented controls visible in the PNG while stripping the `Customize widgets` and `Share` actions so the capture stays focused on the calendar surface
- the trading calendar hover previews now regroup repeated same-symbol trades behind `x2` / `x3` chips in month and week views, and those multiplier chips use the same compact pill height as the P&L badge so the hover rows stay visually aligned
...
- chart widgets own their own section toolbar; the picker stays available even for one-day histories, quick range actions and month navigation only appear when the selected account history actually offers alternate days or months, the chart section now has its own presets control and section-level share export, enabled chart cards are no longer capped to the old 12-widget ceiling, and saved chart layouts accept the full current chart catalog including `Hold Time vs P&L`; the section-level chart PNG keeps the date controls visible while stripping `Presets`, `Share`, and `Customize widgets` from the capture
- chart widgets that display money values now resolve the selected account currency automatically and use net P&L rather than raw trade profit where the chart data path exposes fees and swap, so Daily net, Performance by asset/day, Equity curve, Drawdown, Heat map, and the money-based analytics charts stay aligned with account-level figures instead of silently defaulting to USD-style output
...
- dashboard widget collections now enter customization mode on widget double-click across the main widget grid and chart widgets, replacing the older hold-to-edit gesture
...
- broker settings now own an account-level breakeven tolerance in pips, and saving that threshold refreshes cached trade outcomes for the account so `/dashboard/trades`, BE counts, and summary win-rate math follow that account’s own scratch-trade definition instead of a hardcoded global value
...
- closed trades now auto-create standard trade-review entries directly in the journal and raise a post-close notification that deep-links into the generated entry, instead of sending those reviews through a separate queue tab
...
- accepted private journal shares now appear inside the normal `Entries` feed as read-only `Shared journal` items, and opening one routes through the approved share reader with its specific shared page preselected rather than trying to mount the editable owner journal route
...
- the journal `Insights` tab now uses goals-style outer/inner surfaces and lighter first-mount loading, so workflow, journal intelligence, and psychology/performance panels share the same nested card language without freezing on the initial pattern-analysis render
...
- journal insight prompts and pattern analysis now respect the currently selected account scope, so `Ask your journal` and detected-pattern output read the same entry slice the rest of the journal page is using instead of always analyzing the user’s full history
...
- the journal calendar now treats the tab body as a full-height flex region, so the month-grid surface and its summary-card sidebar both stretch to the full available page height and stay aligned instead of stopping at a smaller fixed desktop cap
- the journal calendar now replaces its older day-detail rail with dashboard-style summary cards for month-level journaling context, including entry totals, active days, busiest day, focus type, latest entry, and selected-day context with a quick path into the newest note on that day
- the journal month grid now distributes its visible weeks across the full calendar height instead of relying on fixed minimum day-card heights, so five-week months no longer leave unused space under the last row
