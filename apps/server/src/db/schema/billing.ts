...
export const privateBetaCode = pgTable(
  "private_beta_code",
  {
    id: text("id")
      .primaryKey()
  ...
  (table) => ({
    codeUnique: uniqueIndex("private_beta_code_code_idx").on(table.code),
    activeIdx: index("private_beta_code_active_idx").on(
      table.isActive,
      table.expiresAt
    ),
  })
...
export const privateBetaWaitlist = pgTable(
  "private_beta_waitlist",
  {
    ...
    status: varchar("status", { length: 24 }).notNull().default("pending"),
    source: varchar("source", { length: 32 }).notNull().default("root"),
    notes: text("notes"),
    invitedCodeId: text("invited_code_id").references(() => privateBetaCode.id, {
      onDelete: "set null",
    }),
    reviewedByUserId: text("reviewed_by_user_id").references(() => user.id, {
      onDelete: "set null",
  ...
  (table) => ({
    emailUnique: uniqueIndex("private_beta_waitlist_email_idx").on(table.email),
    statusIdx: index("private_beta_waitlist_status_idx").on(
      table.status,
      table.createdAt
    ),
  })
...
export const privateBetaRedemption = pgTable(
  "private_beta_redemption",
  {
    id: text("id")
      .primaryKey()
      .$defaultFn(() => crypto.randomUUID()),
    codeId: text("code_id")
      .notNull()
      .references(() => privateBetaCode.id, { onDelete: "cascade" }),
    userId: text("user_id")
      .notNull()
      .references(() => user.id, { onDelete: "cascade" }),
    email: text("email"),
  ...
  (table) => ({
    userUnique: uniqueIndex("private_beta_redemption_user_idx").on(table.userId),
    codeUserUnique: uniqueIndex("private_beta_redemption_code_user_idx").on(
      table.codeId,
      table.userId
    ),
  })
