...
export default function EASetupPage() {
  ...
  const steps = [
    ...
    { title: "Install EA in MT5", completed: hasMT5Account && !!activeKey?.lastUsedAt },
    { title: "Configure EA Settings", completed: hasMT5Account && !!activeKey?.lastUsedAt },
    { title: "Verify Connection", completed: hasMT5Account && !!activeKey?.lastUsedAt },
  ];
    ...
    <div className="flex flex-col w-full">
      ...
      <div className="px-6 sm:px-8 py-5">
        ...
        <p className="text-xs text-white/40 mt-0.5">
          Install the profitabledge data bridge EA for 100% accurate drawdown analysis.
        </p>
      </div>
        ...
        <div className="space-y-2.5">
          {steps.map((step, index) => (
            <div key={index} className="flex items-center gap-2.5">
              ...
              <span className={step.completed ? "text-xs text-white/70" : "text-xs text-white/40"}>
                {step.title}
              </span>
            </div>
          ))}
        ...
        {activeKey ? (
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-yellow-300 text-sm font-medium">No API Key</span>
            </div>
            <p className="text-xs text-white/40">Generate one in API Keys settings.</p>
            <Link href="/dashboard/settings/api">
              <Button className="border border-white/5 bg-teal-600/25 hover:bg-teal-600/35 px-4 py-2 h-[32px] w-max text-xs text-teal-300 cursor-pointer gap-2 transition-all active:scale-95 duration-250">
                Go to API Keys
              </Button>
            </Link>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[200px_1fr] items-start gap-2 sm:gap-6 px-6 sm:px-8 py-5">
        ...
        <div className="space-y-3">
          <Button
            ...
            className="border border-white/5 bg-teal-600/25 hover:bg-teal-600/35 px-4 py-2 h-[38px] w-max text-xs text-teal-300 cursor-pointer gap-2 transition-all active:scale-95 duration-250"
          >
            <Download className="size-3.5" />
            Download profitabledge_data_bridge.mq5
          </Button>
          <p className="text-xs text-white/30">
            MT4 support coming soon. For now, use MT5 or stick with Dukascopy data.
          </p>
        </div>
      </div>
      ...
      {isLocalhost && (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-[200px_1fr] items-start gap-2 sm:gap-6 px-6 sm:px-8 py-5">
            <div>
              <Label className="text-sm text-white/80 font-medium">Install ngrok</Label>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-white/30">
                Or download from{" "}
                <a href="https://ngrok.com/download" target="_blank" rel="noopener noreferrer" className="text-teal-400 hover:text-teal-300">
                  ngrok.com/download
                </a>
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-[200px_1fr] items-start gap-2 sm:gap-6 px-6 sm:px-8 py-5">
            <div>
              <Label className="text-sm text-white/80 font-medium">Start tunnel</Label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-[200px_1fr] items-start gap-2 sm:gap-6 px-6 sm:px-8 py-5">
            <div>
              <Label className="text-sm text-white/80 font-medium">Ngrok URL</Label>
              <p className="text-xs text-white/40 mt-0.5">Paste your forwarding URL.</p>
            </div>
            <div className="space-y-2">
      ...
      {[
        { step: "A", title: "Open Data Folder", desc: "In MT5: File → Open Data Folder" },
        { step: "B", title: "Navigate to Experts", desc: "Go to: MQL5 → Experts folder" },
        { step: "C", title: "Copy EA File", desc: "Copy profitabledge_data_bridge.mq5 into this folder" },
        { step: "D", title: "Refresh Navigator", desc: "Right-click Navigator → Refresh (or restart MT5)" },
      ].map((item, idx) => (
        <div key={item.step}>
          <div className="grid grid-cols-1 sm:grid-cols-[200px_1fr] items-start gap-2 sm:gap-6 px-6 sm:px-8 py-5">
            <div className="flex items-center gap-2.5">
              <Badge className="bg-teal-900/30 text-teal-400 border-teal-500/30 shrink-0 size-6 flex items-center justify-center p-0 text-xs">
                {item.step}
              </Badge>
              <Label className="text-sm text-white/80 font-medium">{item.title}</Label>
            </div>
      ...
      <div className="px-6 sm:px-8 py-5">
        ...
        <p className="text-xs text-white/40 mt-0.5">
          Enable WebRequest and set your API credentials.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[200px_1fr] items-start gap-2 sm:gap-6 px-6 sm:px-8 py-5">
        <div>
          <Label className="text-sm text-white/80 font-medium">Enable WebRequest</Label>
          <p className="text-xs text-white/40 mt-0.5">Required for EA communication.</p>
        </div>
        <div className="space-y-3">
          <ul className="list-disc list-inside text-xs text-white/50 space-y-1">
            ...
            <li>Add <strong className="text-white/70">only the base URL</strong> (without /trpc/webhook...)</li>
          </ul>

          <div className="flex gap-3 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-md">
            <AlertCircle className="h-4 w-4 text-yellow-400 flex-shrink-0 mt-0.5" />
            <div className="text-xs">
              <p className="text-yellow-300 font-medium mb-1">Add base URL only!</p>
              <p className="text-white/40">
                Correct: <code className="text-teal-400 font-mono">{apiUrl}</code>
              </p>
              <p className="text-white/40 mt-0.5">
                Wrong: <code className="text-red-400/60 font-mono line-through">{webhookUrl}</code>
              </p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <code className="flex-1 text-xs bg-sidebar-accent border border-white/5 px-3 py-2 rounded text-white/70 font-mono">
              {apiUrl}
            </code>
            <Button
              size="sm"
              onClick={() => handleCopyUrl(apiUrl, "base")}
              className="border border-white/5 bg-teal-600/25 hover:bg-teal-600/35 text-teal-300 h-8 w-8 p-0"
            >
              {copiedBaseUrl ? <Check className="size-3.5" /> : <Copy className="size-3.5" />}
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[200px_1fr] items-start gap-2 sm:gap-6 px-6 sm:px-8 py-5">
        <div>
          <Label className="text-sm text-white/80 font-medium">EA Input Values</Label>
          <p className="text-xs text-white/40 mt-0.5">Set these when attaching EA to chart.</p>
        </div>
        <div className="bg-sidebar-accent border border-white/5 rounded-md p-3">
          <table className="w-full text-xs">
            <tbody>
              ...
              <tr>
                ...
                <td className="text-white/70 font-mono text-xs break-all">{webhookUrl}</td>
              </tr>
              <tr>
                <td className="text-white/40 py-1.5 pr-4">UPDATE_INTERVAL_MS:</td>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[200px_1fr] items-start gap-2 sm:gap-6 px-6 sm:px-8 py-5">
        <div>
          <Label className="text-sm text-white/80 font-medium">Connection Status</Label>
        ...
        {activeKey?.lastUsedAt ? (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <CheckCircle2 className="size-4 text-teal-400" />
              <span className="text-teal-300 text-sm font-medium">EA Connected</span>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <AlertCircle className="size-4 text-yellow-400" />
              <span className="text-yellow-300 text-sm font-medium">Waiting for EA</span>
            </div>
            <div className="bg-sidebar-accent border border-white/5 p-3 rounded-md font-mono text-xs text-white/60 space-y-0.5">
              <div>======================================</div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[200px_1fr] items-start gap-2 sm:gap-6 px-6 sm:px-8 py-5">
        <div>
          <Label className="text-sm text-white/80 font-medium">Need Help?</Label>
