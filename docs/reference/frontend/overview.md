...
- the root route is now the primary marketing and invite-entry surface, while sign-up and onboarding flow directly into account creation and plan selection without a beta access gate
- onboarding plan selection now stays inside the existing `(onboarding)` shell while reading live billing config from tRPC, starting Stripe checkout for paid plans, redeeming stored referral or affiliate invite intent, and restoring the user to the account-setup step after checkout instead of resetting the flow
...
- the onboarding route now calls billing sync on paid-checkout return, with a short retry window for local-dev or webhook-lag scenarios, so the selected plan is mirrored into app billing state before the user finishes onboarding
- the onboarding route now self-skips for completed users instead of replaying the plan flow unnecessarily
...
- onboarding now waits for any stored referral or affiliate access intent from sign-up before bootstrapping the current step, so brand-new users do not get pushed from `Profile` to `Select a plan` at the wrong time
...
- admin-only growth routes such as `/dashboard/growth-admin` remain restricted to staff accounts, while launch-facing dashboard routes are no longer held behind beta access logic
...
- the journal entries feed now opts into accepted shared journals, rendering them as read-only `Shared journal` rows/cards and routing selection through the share reader path with the requested entry preselected instead of opening the editable owner-entry page
