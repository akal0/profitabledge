...
export const aggregatedStatsProcedure = protectedProcedure.query(async ({ ctx }) => {
  ...
  for (const row of trades) {
    ...
    const profit = parseAccountNumber(row.profit);
  ...
  return {
    ...
    totals: {
      ...
      overallWinRate:
        totalTrades > 0 ? roundAccountNumber((totalWins / totalTrades) * 100, 1) : 0,
      overallExpectancy:
