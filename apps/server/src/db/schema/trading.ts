...
export const tradingAccount = pgTable("trading_account", {
  ...
  verificationLevel: varchar("verification_level", { length: 20 }).default("unverified"), // unverified, ea_synced, api_verified, prop_verified
...
export const trade = pgTable("trade", {
  ...
  sessionTagIdx: index("idx_trade_session_tag").on(table.accountId, table.sessionTag),
  modelTagIdx: index("idx_trade_model_tag").on(table.accountId, table.modelTag),
  protocolAlignmentIdx: index("idx_trade_protocol_alignment").on(table.accountId, table.protocolAlignment),
...
export const openTrade = pgTable("open_trade", {
  ...
  uniqueIndex("open_trade_account_ticket_idx").on(table.accountId, table.ticket),
...
export const tradeView = pgTable("trade_view", {
...
export const goal = pgTable("goal", {
  ...
  userId: text("user_id")
    ...
    .references(() => user.id, { onDelete: "cascade" }),
  accountId: text("account_id")
    .references(() => tradingAccount.id, { onDelete: "cascade" }), // null = all accounts
...
export const propFirm = pgTable("prop_firm", {
  id: text("id").primaryKey(),
...
export const propChallengeRule = pgTable("prop_challenge_rule", {
  id: text("id").primaryKey(),
...
export const propDailySnapshot = pgTable("prop_daily_snapshot", {
  id: text("id").primaryKey(),
  accountId: text("account_id")
    .notNull()
    .references(() => tradingAccount.id, { onDelete: "cascade" }),
  ...
  accountDateIdx: uniqueIndex("idx_prop_daily_snapshot_account_date").on(table.accountId, table.date),
...
export const performanceAlertRule = pgTable("performance_alert_rule", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  userId: text("user_id")
    .notNull()
    .references(() => user.id, { onDelete: "cascade" }),
  accountId: text("account_id")
    .references(() => tradingAccount.id, { onDelete: "cascade" }), // null = applies to all accounts
  ...
  alertSeverity: varchar("alert_severity", { length: 20 }).notNull().default("warning"), // "info" | "warning" | "critical"
...
export const performanceAlert = pgTable("performance_alert", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  userId: text("user_id")
    .notNull()
    .references(() => user.id, { onDelete: "cascade" }),
  accountId: text("account_id")
    .references(() => tradingAccount.id, { onDelete: "cascade" }),
  ruleId: text("rule_id")
    .references(() => performanceAlertRule.id, { onDelete: "set null" }),
...
export const propAlert = pgTable("prop_alert", {
  id: text("id").primaryKey(),
  accountId: text("account_id")
    .notNull()
    .references(() => tradingAccount.id, { onDelete: "cascade" }),
  ...
  accountSeverityIdx: index("idx_prop_alert_account_severity").on(table.accountId, table.severity),
}));

// ============== TRADING RULES ENGINE ==============

// Trading Rule Sets - User-defined rule configurations
export const tradingRuleSet = pgTable("trading_rule_set", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  userId: text("user_id")
    .notNull()
    .references(() => user.id, { onDelete: "cascade" }),
...
export const tradeRuleEvaluation = pgTable("trade_rule_evaluation", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
...
export const pnlCardTemplate = pgTable("pnl_card_template", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  ...
  backgroundType: varchar("background_type", { length: 20 }).notNull().default("gradient"), // 'gradient' | 'image' | 'solid'
...
export const sharedPnlCard = pgTable("shared_pnl_card", {
  id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
  ...
  tradeId: text("trade_id")
    .notNull()
    .references(() => trade.id, { onDelete: "cascade" }),
  templateId: text("template_id").references(() => pnlCardTemplate.id, { onDelete: "set null" }),
...
export const propFirmRelations = relations(propFirm, ({ many }) => ({
  challengeRules: many(propChallengeRule),
}));

export const propChallengeRuleRelations = relations(propChallengeRule, ({ one }) => ({
...
export const propDailySnapshotRelations = relations(propDailySnapshot, ({ one }) => ({
  account: one(tradingAccount, {
